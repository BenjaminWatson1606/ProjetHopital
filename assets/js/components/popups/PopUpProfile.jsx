...
function PopUpProfile(item) {
	...
	function changeSkillLevel(id, level) {
		skillsAPI.updateSkillLevel(id, level)
				...
				for (let j = 1; j <= i; j++) {
					...
					listInterviews.push(
						<div id={j} className={classe}>
							...
							<div className="ligne"><strong>{t("nextDateMeeting")} :</strong>{new Date(interviews[j - 1].DateNextInterview).toISOString().split('T')[0]}</div>
						</div>
					);
				}
				for (let h = i + 1; h <= 3; h++) {
	...
	const blockedState = ({ currentTarget }) => {
		const state = currentTarget.checked;
		recruitmentsAPI.updateBlockedState(profile.recruitment.id, state)
		window.location.reload(false);
	}
	...
	return (
		<>
			...
			<ul className="nav">
				...
				<li id="skill" onClick={() => switchView("skill", "divskill")}>{t("skills")}</li>
				<li id="contact" onClick={() => switchView("contact", "divcontact")}>Contact</li>
				<li id="state" onClick={() => switchView("state", "divstate")}>{t("recruitmentState")}</li>
			</ul>
			<div id="divcv">
			...
			<div id="divstate">
				...
				<div className="blocked">
					<Field
						...
						checked={profile.recruitment.BlockedState}
						value={profile.recruitment.BlockedState}
						type="checkbox"
					/>
				</div>
				<div className="TakeContact">
					<h2>{t("contactInformations")} :</h2>
					<strong>{t("dateContact")} :</strong>{new Date(profile.recruitment.ContactDate).toISOString().split('T')[0]}
					<br />
					<strong>{t("contactMethod")} :</strong>{profile.recruitment.ContactMethod}
				</div>
				<div className="interviewList">
					<h2>{t("interviewsInformations")} :</h2>
					{getInterviews(profile.recruitment.interviews)}
				</div>
				<div className="contractinfo">
					<h2>{t("contractInformations")} :</h2>
					<strong>{t("contractSigned")} : </strong>{new Date(profile.recruitment.ContractDate).toISOString().split('T')[0]}
					<br />
					<strong>{t("contractSuppInformations")} : </strong>{profile.recruitment.ContractDescription}
				</div>
			</div>
		</>
	);
